...
void Graph::AStar(pairType start, map<pairType,pairType> path, pairType goal, map<pairType,int> dist)
{
	...
	vector<pairType> adj;
	map<pairType,bool> visited;
	//initialize start path and neighbors and populate the heap
	dist[start]=0;
	int cost= dist[start] + heuristic(start,goal);
	addPath(start,start,path);
	neighbors(start, adj);
	initializeNeighborgs(adj,visited,dist);
	
	
	pq.push(make_pair(cost,start));
		cout << "this is a test: " << endl; printMap(dist);
		
		//find next tile
		pairType bestGuess = pq.top().second;
		pq.pop();
		calculateBestDistance(bestGuess,adj,dist);
		if(adj.empty())
		{
			cout << "true" << endl;
		}
...
void Graph::calculateBestDistance(pairType bestGuess ,vector<pairType> &adj,map<pairType,int> &dist)
{
	for(auto i : adj)
			{
...
void Graph::initializeNeighborgs(vector<pairType> &adj, map<pairType,bool> &visited, map<pairType,int> &dist)
{
	for(auto i : adj)
	{

	if((visited.find(i)==visited.end()) && (dist.find(i) == dist.end()))
		{
...
void Graph::neighbors(pairType adj, vector<pairType> &valid){
	...
	void Graph::printMap(map<pairType,pairType> &path)
	{
	for(auto i : path)
	{
		cout <<"Key: " << i.first.first << ", " << i.first.second << endl <<"Val: " << i.second.first << ", " << i.second.second << endl;
	}	
	}
	void Graph::printMap(map<pairType,int> &path)
	{
	for(auto i : path)
	{
		cout <<"Key: " << i.first.first << ", " << i.first.second << endl <<"Val: " << i.second <<  endl;
	}	
	}
